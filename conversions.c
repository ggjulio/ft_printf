...
/*   Updated: 2019/11/17 18:00:35 by juligonz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

long long	cast_len(t_manager *p, va_list *args, void *ptr)
{
	
	if (GET(F_H))
		ptr = va_arg(*args, short);
	if (GET(F_HH))
		ptr = va_arg(*args, signed char);
	if (GET(F_L))
		ptr = va_arg(*args, long);
	if (GET(F_LL))
		ptr = va_arg(*args, long long);
...
{
	long long l;
//	ft_putnbr_base_fd(cast_len(p, args), 10, p);
	cast_len(p, args, l);
...
{
	ft_putnbr_base_fd(cast_len(va_arg(*args, int)), 10, p);
}
